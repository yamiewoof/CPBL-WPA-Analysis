...
#創造可以算wpa的欄位
# forbatter <- analysisdataADJ
# forbatter$TeamBat4WPA <- forbatter$BatterTeam
# forbatter$TeamPit4WPA <- forbatter$PitcherTeam
# 
# for (i in 2:nrow(forbatter)){
#   if (i%%100==0){
#     print(i)
#   }
#     if ( (forbatter$inning[i] != forbatter$inning[i-1] ) & (forbatter$inning[i]!= "1上") ){
# 
#       forbatter$TeamBat4WPA[i] <- forbatter$TeamBat4WPA[i-1]
#       forbatter$TeamPit4WPA[i] <- forbatter$TeamPit4WPA[i-1]
# 
#     }
# 
#   if ( (forbatter$inning[i] != forbatter$inning[i-1] ) & (forbatter$inning[i]== "1上") ){
# 
#     forbatter$TeamBat4WPA[i] <- NA
#     forbatter$TeamPit4WPA[i] <- NA
# 
#   }
# 
# 
...
#forbatter <- forbatter %>% select(1:7,TeamBat4WPA,TeamPit4WPA,11:20)
...
ggplot(TeamPitWPAsum, aes(x=Team, y=WPAsum)) + 
  ...
  labs(title="PitchingWPAsum", 
       subtitle="Data:2018") + 
  theme(axis.text.x = element_tex t(angle=0, vjust=0.6))
